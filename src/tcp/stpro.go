...
func (srv *Server) handle(conn net.Conn) {
	...
	dePacketClient(conn, &taskChan)
	//	if v, ok := remoteBufMap[conn.RemoteAddr().String()]; ok {
	//		buf, err := dePacket(conn, &taskChan, v)
	//	} else {
	//		buf, err := dePacket(conn, &taskChan, nil)
	//	}
	//	buf, err := dePacket(conn, &taskChan, remoteBufMap[conn.RemoteAddr().String()])
	//	if err != nil && len(buf) != 0 {
	//		remoteBufMap[conn.RemoteAddr().String()] = buf
	//	}
	return
}
